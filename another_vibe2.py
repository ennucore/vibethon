...
def instrument_function(func):
    ...
    and calls breakpoint(), attempting to preserve line numbers for debuggers.
    NOTE: Due to Python's AST and code object limitations, perfect mapping for
    pdb's `list` command is not possible when instrumenting at the statement level.
    """
    import textwrap
    import inspect
    import ast
    ...
    # Remove decorator lines if present
    ...
    # Try to preserve line numbers by setting the lineno of the function definition
    func_def.lineno = start_line

    new_body = []
    for stmt in func_def.body:
        # Wrap each statement in a try/except, but preserve the original line numbers
        try_node = ast.Try(
            body=[stmt],
        ...
        # Set the try node's line number to match the wrapped statement
        if hasattr(stmt, 'lineno'):
            try_node.lineno = stmt.lineno
            try_node.col_offset = getattr(stmt, 'col_offset', 0)
        new_body.append(try_node)

    func_def.body = new_body
    ast.fix_missing_locations(tree)
    # Compile with the correct filename and starting line number
